...
REAL_HEIGHTS = {
    ...
    "bicycle": 1.4,       # Average height of a bicycle
    "car": 1.5,           # Average height of a car
    "motorcycle": 1.4,    # Average height of a motorcycle
...
def real_time_inference(args):
    ...
    alert_distance = args.alert_distance # distance threshold for alerts (in meters)
    ...
    while True:
        ...
        alert_messages = [] # list to hold alert messages for this frame
        ...
        for (prob, label_with_id), (xmin, ymin, xmax, ymax) in zip(scores, boxes):
            ...
            # check if object is within alert distance and is not a person
            if is_alert and distance < alert_distance:
                alert_messages.append(f"WARNING! {label.capitalize()} approaching at {total_speed_kmh:.2f}km/h, {distance:.2f}m away")

                # Create an overlay for semi-transparent rectangle
                overlay = frame.copy()
                # define semi-transparent red color in BGR (OpenCV uses BGR format)
                semi_transparent_red = (0, 0, 255) # red color
                alpha = 0.4 # transparency factor

                # draw filled rectangle on the overlay
                cv2.rectangle(overlay, (int(xmin), int(ymin)), (int(xmax), int(ymax)), semi_transparent_red, thickness=-1)

                # blend the overlay with the original frame
                cv2.addWeighted(overlay, alpha, frame, 1 - alpha, 0, frame)

                # optionally, redraw the bounding box border after blending
                cv2.rectangle(frame, (int(xmin), int(ymin)), (int(xmax), int(ymax)), (0, 255, 255), 2) # Original border color

            # draw bounding boxes
            if not (is_alert and distance < alert_distance):
        ...
        # display alert messages at the bottom of the frame
        if alert_messages:
            # Define starting position for alerts
            alert_x = 10
            alert_y_start = frame.shape[0] - 20  # 20 pixels from the bottom
            line_height = 20  # Space between lines

            for i, message in enumerate(alert_messages):
                # Calculate y position for each message
                alert_y = alert_y_start - i * line_height
                ...
                cv2.rectangle(frame, (alert_x - 5, alert_y - msg_height - 5), 
                                      (alert_x + msg_width + 5, alert_y + 5), 
                                      (0, 0, 255), cv2.FILLED)
                # Put text
                cv2.putText(frame, message, (alert_x, alert_y), font, font_scale, (255, 255, 255), thickness)
...
if __name__ == '__main__':
    ...
    # distance threshold for alerts in meters
    parser.add_argument('--alert_distance', default=30.0, type=float)
    ...
    # video file path
    parser.add_argument('--video_path', default='/Users/sekwang/Downloads/IMG_0529.MOV', type=str, help="Path to the input video file (e.g., .MOV, .MP4)")
