...
# for object detection - swin_s (remove mlp head layer)
def swin_t_for_detection(hidden_dim=96, layers=(2, 2, 6, 2), heads=(3, 6, 12, 24), for_detection=True, **kwargs):
    return SwinTransformer(hidden_dim=hidden_dim, layers=layers, heads=heads, for_detection=for_detection, **kwargs)


# for object detection - swin_b (remove mlp head layer)
def swin_b_for_detection(hidden_dim=128, layers=(2, 2, 18, 2), heads=(4, 8, 16, 32), for_detection=True, **kwargs):
    return SwinTransformer(hidden_dim=hidden_dim, layers=layers, heads=heads, for_detection=for_detection, **kwargs)